...
pthread_mutex_t mutexes[THREAD_COUNT];  // Matriz de mutexes para cada hilo
...
void* Send_msg(void* rank) {
    ...
    pthread_mutex_lock(&mutexes[dest]);         // 2
    messages[dest] = my_msg;                     // 4
    pthread_mutex_unlock(&mutexes[dest]);       // 5

    // Intentar leer el mensaje asignado a este hilo
    pthread_mutex_lock(&mutexes[my_rank]);       // 7
    if (messages[my_rank] != NULL)
        printf("Thread %ld > %s\n", my_rank, messages[my_rank]);
    else
        printf("Thread %ld > No message from %ld\n", my_rank, source);
    pthread_mutex_unlock(&mutexes[my_rank]);     // 9
...
int main() {
    ...
    for (thread = 0; thread < THREAD_COUNT; thread++) {
        ...
        pthread_mutex_init(&mutexes[thread], NULL);  // Inicializar cada mutex
    }
    ...
    for (thread = 0; thread < THREAD_COUNT; thread++) {
        ...
        pthread_mutex_destroy(&mutexes[thread]);  // Destruir cada mutex
    }
